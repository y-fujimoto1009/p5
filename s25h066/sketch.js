...
// 小円アニメーション（左右から）
  ...
  let leftY = lerp(0, 200, min(step / 60, 20));
  let rightX = lerp(400, 250, min(step / 60, 1));
  let rightY = lerp(400, 200, min(step / 60, 20));
  let leftZ = lerp(400, 150, min(step / 60, 1));
  let leftQ = lerp(0, 200, min(step / 60, 1));
  let rightW = lerp(0, 250, min(step / 60, 1));
  let rightI = lerp(400, 200, min(step / 60, 1));
circle(leftX, leftY , 50);
circle(rightX,rightY, 50);
circle(leftZ, leftQ , 50);
circle(rightW,rightI, 50);
// 大円アニメーション（拡大）
